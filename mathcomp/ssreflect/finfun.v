...
Local Fixpoint fun_of_fin_rec x s (f_s : finfun_on s) : x \in s -> rT x :=
  if f_s is finfun_cons x1 s1 y1 f_s1 then
    if eqP is ReflectT Dx in reflect _ Dxb return Dxb || (x \in s1) -> rT x then
      fun=> ecast x (rT x) (equality.eq_sym Dx) y1
...
suffices ffunK f g: (forall x, f x = g x) -> f = finfun g.
  by split=> [/ffunK|] -> //; apply/equality.eq_sym/ffunK.
...
(* FIXME: finfun_eqType should be inferred. *)
Canonical finfun_choiceType (rT : choiceType) :=
  @Choice.pack {ffun aT -> rT} (choiceMixin (fun=> rT)) _ (finfun_eqType rT) id.
Fail Canonical original_finfun_choiceType (rT : choiceType) :=
  ChoiceType {ffun aT -> rT} (choiceMixin (fun=> rT)).
(* FIXME: dfinfun_eqType should be inferred. *)
Canonical dfinfun_choiceType rT :=
  @Choice.pack (dffun_aT rT choiceType) (choiceMixin rT) _ (dfinfun_eqType rT) id.
Fail Canonical original_dfinfun_choiceType rT :=
...
(* FIXME: finfun_choiceType should be inferred. *)
Canonical finfun_countType (rT : countType) :=
  @Countable.pack {ffun aT -> rT} (countMixin (fun => rT)) (finfun_choiceType rT) _ id.
Fail Canonical original_finfun_countType (rT : countType) :=
  CountType {ffun aT -> rT} (countMixin (fun => rT)).
(* FIXME: dfinfun_choiceType should be inferred. *)
Canonical dfinfun_countType rT :=
  @Countable.pack (dffun_aT rT countType) (countMixin rT) (dfinfun_choiceType rT) _ id.
Fail Canonical original_dfinfun_countType rT :=
  CountType (dffun_aT rT countType) (countMixin rT).

Local Definition finMixin (rT: aT -> finType) :=
  @PcanFinMixin (dfinfun_countType rT) _ _ _ (tfgraphK : @pcancel _ (dffun_aT rT finType) _ _).
Fail Local Definition original_finMixin rT :=
  PcanFinMixin (tfgraphK : @pcancel _ (dffun_aT rT finType) _ _).
(* FIXME: finfun_choiceType should be inferred. *)
Canonical finfun_finType (rT : finType) :=
  @Finite.pack {ffun aT -> rT} _ (finMixin (fun=> rT)) (finfun_choiceType rT) _ id _ id.
Fail Canonical original_finfun_finType (rT : finType) :=
  FinType {ffun aT -> rT} (finMixin (fun=> rT)).
(* FIXME: dfinfun_choiceType should be inferred. *)
Canonical dfinfun_finType rT :=
  @Finite.pack (dffun_aT rT finType) _ (finMixin rT) (dfinfun_choiceType rT) _ id _ id.
Fail Canonical original_dfinfun_finType rT :=
...
Proof. exact/equality.eq_sym/val_inj/codom_ffun. Qed.
...
transitivity (#|[predX F x0 & family F1 : pred fT]|); last first.
  rewrite cardX {}IH_E {uniqE}// => [|x E'x]; last first.
    rewrite /F1; case: eqP => [Dx | /neq_sym/eqP-x0'x]; first exact: card1.
...
apply/imageP/andP=> [[f1 /familyP/=Ff1] /pairI[-> ->]| [/=Fx0y /familyP/=Ff]].
...
Proof. by rewrite -card_family; apply/equality.eq_sym/eq_card=> f; apply/familyP. Qed.
...
Proof. by rewrite -card_ffun_on; apply/equality.eq_sym/eq_card=> f; apply/forallP. Qed.
